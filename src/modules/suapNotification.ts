...
export default class suapNotification extends Module {
    ...
    async startSemestreLoader() {
        return new Promise(async superRes => {
            ...
            const users = this.client.users.map(u => u) as User[];
    ...
    async turmas() {
        ...
        const func = async () => {
            ...
            for (const user of Object.values(users)) {
                if (!user.periodo) continue;

                await new Promise(async resolve => {
                    let turmas = await this.client.modules.get('suap').minhasTurmas(user.token, user.periodo.ano_letivo, user.periodo.periodo_letivo);
                    ...
                    if (!turmas) {
                        ...
                        turmas = await this.client.modules.get('suap').minhasTurmas(login.access, user.periodo.ano_letivo, user.periodo.periodo_letivo).catch(() => false);
